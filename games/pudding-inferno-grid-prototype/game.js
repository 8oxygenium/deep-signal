...
function generateDungeon() {
  ...
  // 敵配置（スタート以外の部屋に1〜2体）
  enemies = [];
  const enemyRooms = rooms.filter(r => r !== startRoom);
  // 2〜3体、間隔を置いた部屋に配置
  const targets = enemyRooms.length >= 2
    ? [enemyRooms[Math.floor(enemyRooms.length / 2)], enemyRooms[enemyRooms.length - 1]]
    : [enemyRooms[0]].filter(Boolean);
  if (rooms.length >= 3) targets.push(enemyRooms[0]);

  for (const rm of targets) {
    if (!rm) continue;
    enemies.push({
      col: rm.cx, row: rm.cy,
      hp: CFG.KNIGHT_HP, hpMax: CFG.KNIGHT_HP,
      warnCol: -1, warnRow: -1,
      alive: true,
      flashTimer: 0,
    });
...
function doEnemyTurns() {
  ...
  for (const e of alive) {
    ...
    if (adjToPlayer) {
      if (e.warnCol === player.col && e.warnRow === player.row) {
        if (player.invincible <= 0) applyDamage();
...
function handleDown(cx, cy) {
  ...
  if (gameState === STATE.DEATH) { if (deathRetryVisible) { initGame(); gameState = STATE.PLAYING; } return; }
...
window.addEventListener('keydown', (e) => {
  ...
  if (gameState === STATE.TITLE) { initGame(); gameState = STATE.PLAYING; return; }
  if (gameState === STATE.CLEAR) { initGame(); gameState = STATE.PLAYING; return; }
  if (gameState === STATE.DEATH && deathRetryVisible) { initGame(); gameState = STATE.PLAYING; return; }
  if (gameState !== STATE.PLAYING) return;
...
function drawTitle(now) {
  ...
  if (blink) {
    ctx.fillStyle='#D4C5A9'; ctx.font='17px sans-serif'; ctx.textAlign='center';
    ctx.fillText('タップ / クリックでスタート', W/2, H*0.76);
  }
  ctx.fillStyle='rgba(212,197,169,0.4)'; ctx.font='12px sans-serif';
  ctx.fillText('ターン制グリッド探索版', W/2, H*0.83);
}
